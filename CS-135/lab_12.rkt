...
(define (composite  relationOuter relationInner)
  (if (null? relationOuter)
      '()
      (append (composite-helper (car relationOuter) relationInner) (composite (cdr relationOuter) relationInner))))

(define (composite-helper tuple inner)
  (if (null? inner)
      '()
      (if (equal? (cadar inner) (car tuple))
          (cons (list (caar inner) (cadr tuple)) (composite-helper tuple (cdr inner)))
      (composite-helper tuple (cdr inner)))))
...
(define (power relation k)
  (if (zero? k)
      '()
      (power-helper relation k)))


(define (power-helper relation k)
  (if (equal? k 1)
      relation
      (power-helper (composite relation relation) (- k 1))))
...
(define (transitive-closure relation)
  (tc-helper relation relation (length relation)))

(define (tc-helper r1 r2 len)
  (if (zero? len)
      '()
      (if (equal? len 1)
          r2
          (union r2 (tc-helper r1 (power r1 len) (- len 1))))))
