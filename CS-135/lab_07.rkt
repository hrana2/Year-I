...
(define (base10-helper oldbase numlist cur)
  (if (null? numlist)
      0
      (+ (* (car numlist) cur) (base10-helper oldbase (cdr numlist) (* cur oldbase)))))


(define (base10 oldbase numlist)
   (base10-helper oldbase (reverse numlist) 1))
...
(define (geometric-prog cur_num ratio n)
  (if (zero? n)
      '()
      (cons cur_num (geometric-prog (* cur_num ratio) ratio (- n 1)))))