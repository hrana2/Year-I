...
(define (CRT-exists? tuplelist)
  (if (null? tuplelist)
      #t
      (if (CTR-exist?-helper (cadar tuplelist) (cdr tuplelist))
          (CRT-exists? (cdr tuplelist))
          #f)))

(define (CTR-exist?-helper int1 tuplelist)
  (if (null? tuplelist)
      #t
      (if (equal? (EuclidAlgo int1 (cadar tuplelist)) 1)
          (CTR-exist?-helper int1 (cdr tuplelist))
          #f)))
